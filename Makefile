.PHONY: all build test clean build-ts build-go test-ts test-go scan-prose clean-ts clean-go \
        publish publish-go check-go-major tags-go reset sync-model check-model model-ref
...
# Parity goldens. go/validate_test.go compares its output against
# ts/test/model-ref/, which is generated by the CANONICAL TypeScript
# implementation — so the comparison is a real parity check rather than a
# Go-vs-Go snapshot. Refresh after any deliberate change to model output, and
# commit the result with the change that caused it.
model-ref: build-ts
	@node ts/build/model-ref.js
