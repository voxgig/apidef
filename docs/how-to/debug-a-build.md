...
to `<prefix>base-guide.aon` first, so you can always open that file to read
...
| `ENOENT … <prefix>guide.aon` | missing guide entry file | create it — [the guide file](../reference/configuration.md#the-guide-file) |
...
| `PATH MISMATCH` | a path/method wasn't classified | `<prefix>base-guide.aon`, the `why_*` traces |
