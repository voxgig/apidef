...
# approach against it. Compares .aon and json output files.
...
compare_outputs() {
  ...
  if [ $jsonic_fail -eq 0 ] && [ $jsonic_pass -gt 0 ]; then
    report "PASS" "$label: all $jsonic_pass .aon files match"
  elif [ $jsonic_fail -gt 0 ]; then
    report "FAIL" "$label: $jsonic_fail/$((jsonic_pass + jsonic_fail)) .aon files differ"
  fi
}
