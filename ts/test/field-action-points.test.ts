...
describe('field-action-points', () => {
  ...
  test('an action point contributes no fields', async () => {
    ...
    assert.deepStrictEqual(names(fields), ['diameter', 'id', 'kind', 'name'],
      'the entity is what the spec says a planet is, not that plus two ' +
      'action payloads and their reply envelope')

    for (const stray of ['forbid', 'ok', 'start', 'state', 'stop', 'why']) {
      assert.ok(!names(fields).includes(stray),
        stray + ' is an action argument or result, never a planet field')
    }
  })
