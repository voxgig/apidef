...
`;
        ...
        (0, node_test_1.test)('a wildcard DEFAULT turns the denylist into an allowlist', () => {
            const out = unify(SHAPE +
                'guide: entity: &: active: *false\n' +
                'guide: entity: card: active: true\n');
            const ent = out.guide.entity;
            node_assert_1.default.strictEqual(ent.card.active, true, 'the named entity stays on');
            node_assert_1.default.strictEqual(ent.payment.active, false, 'an unnamed entity goes off');
            node_assert_1.default.strictEqual(ent.envelope.active, false, 'and so does every other');
        });
        (0, node_test_1.test)('a CONCRETE wildcard value cannot be overridden', () => {
            ...
            // wins.
            node_assert_1.default.throws(() => unify(SHAPE +
                'guide: entity: &: active: false\n' +
                'guide: entity: card: active: true\n'));
        });
    });
