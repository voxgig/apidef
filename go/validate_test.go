...
func TestValidateModelData(t *testing.T) {
	...
	defDir := filepath.Join(validateDir, "..", "def")
	// THE REFERENCES ARE THE CANONICAL TYPESCRIPT'S OUTPUT, written by
	// ts/build/model-ref.js. That makes this a genuine parity check; it used
	// to create them from Go's own output on first run, which made it a
	// Go-vs-Go regression snapshot whose home under ts/test/ said otherwise.
		...
		t.Run("data-"+cn, func(t *testing.T) {
			...
			for _, entName := range entNames {
				...
				if _, err := os.Stat(refFile); os.IsNotExist(err) {
					// NOT CREATED FROM GO OUTPUT. Writing the reference here
					// is what made this a Go-vs-Go snapshot: whatever this
					// port produced first became the thing it was compared
					// against, so a divergence from the canonical TS was
					// recorded as correct. The references are generated by
					// the TypeScript implementation — see
					// ts/build/model-ref.js — and a missing one is a missing
					// refresh, not a licence to invent it.
					t.Errorf("no reference for %s at %s — refresh the "+
						"goldens from the canonical implementation: "+
						"node ts/build/model-ref.js", entName, refFile)
					continue
				}
