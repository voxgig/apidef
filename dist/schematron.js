...
function convert(schema, opts = {}) {
    const seen = new Set(); // avoid accidental infinite loops on bad refs
    const resolveRef = ($ref) => {
        if (!$ref.startsWith("#/"))
            return undefined; // non-local refs not supported per spec; return undefined to fallback
        const parts = $ref.slice(2).split("/").map(unescapeRefToken);
        let node = opts.rootDoc ?? {};
...
function includesNullViaOneAnyOf(s) {
    const hasNullIn = (arr) => Array.isArray(arr) &&
...
function mergeAllOf(schemas) {
    ...
    return anyObject ? out : schemas.reduce((a, b) => ({ ...a, ...b }), {});
}
/* -------------------------- Example usage --------------------------
